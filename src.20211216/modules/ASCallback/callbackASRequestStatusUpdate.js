module.exports.NAME = async function(req, res, next) {
  const headersReqSchema = this.utils().
      schemas('req.callbackASRequestStatusUpdateSchema.headersSchema');
  const bodyReqSchema = this.utils().
      schemas('req.callbackASRequestStatusUpdateSchema.bodySchema');
  ...
  const nodeCmd = 'callback_as_request_status_update';
